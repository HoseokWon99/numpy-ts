  ...
  for (const dtype of FLOAT) {
    it(`linalg.norm ${dtype}`, () => {
      ...
      const pyResult = runNumPy(
        `result = float(np.linalg.norm(np.array([3, 4], dtype=${npDtype(dtype)})))`
      );
      expect(Number(jsResult)).toBeCloseTo(Number(pyResult.value), 4);
    });

    it(`linalg.det ${dtype}`, () => {
      const jsResult = np.linalg.det(
        array(
          [
            [1, 2],
            [3, 4],
      ...
      const pyResult = runNumPy(
        `result = float(np.linalg.det(np.array([[1,2],[3,4]], dtype=${npDtype(dtype)})))`
      );
      expect(Number(jsResult)).toBeCloseTo(Number(pyResult.value), 4);
    });

    it(`linalg.inv ${dtype}`, () => {
      const jsResult = np.linalg.inv(
        array(
          [
            [1, 2],
            [3, 4],
      ...
      const pyResult = runNumPy(
        `result = np.linalg.inv(np.array([[1,2],[3,4]], dtype=${npDtype(dtype)})).astype(np.float64)`
      );
      expect(arraysClose(jsResult.toArray(), pyResult.value, 1e-4)).toBe(true);
    });

    it(`linalg.cholesky ${dtype}`, () => {
      const jsResult = np.linalg.cholesky(
        array(
          [
            [4, 2],
            [2, 5],
      ...
      const pyResult = runNumPy(
        `result = np.linalg.cholesky(np.array([[4,2],[2,5]], dtype=${npDtype(dtype)})).astype(np.float64)`
      );
      expect(arraysClose(jsResult.toArray(), pyResult.value, 1e-4)).toBe(true);
    });

    it(`linalg.qr ${dtype}`, () => {
      const { q, r } = np.linalg.qr(
        array(
          [
            [1, 2],
            [3, 4],
...
result = s.astype(np.float64)
    ...
    it(`linalg.solve ${dtype}`, () => {
      const jsResult = np.linalg.solve(
        array(
          [
            [3, 1],
            [1, 2],
      ...
      const pyResult = runNumPy(
        `result = np.linalg.solve(np.array([[3,1],[1,2]], dtype=${npDtype(dtype)}), np.array([9,8], dtype=${npDtype(dtype)})).astype(np.float64)`
      );
      expect(arraysClose(jsResult.toArray(), pyResult.value, 1e-4)).toBe(true);
    });

    it(`linalg.lstsq ${dtype}`, () => {
      const { x } = np.linalg.lstsq(
        array(
          [
            [1, 1],
            [1, 2],
    ...
    it(`linalg.matrix_rank ${dtype}`, () => {
      const jsResult = np.linalg.matrix_rank(
        array(
          [
            [1, 2],
            [3, 4],
    ...
    it(`linalg.slogdet ${dtype}`, () => {
      const { sign, logabsdet } = np.linalg.slogdet(
        array(
          [
            [1, 2],
            [3, 4],
...
result = np.kron(a, b).astype(np.float64)
    ...
    it(`trace ${dtype}`, () => {
      ...
      const pyResult = runNumPy(
        `result = float(np.trace(np.array([[1,2],[3,4]], dtype=${npDtype(dtype)})))`
      );
      expect(Number(jsResult)).toBeCloseTo(Number(pyResult.value), 4);
    });
