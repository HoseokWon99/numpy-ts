...
import { sinh_f64, sinh_f32, sinh_i64, sinh_u64 } from './bins/sinh.wasm';
import {
  ensureMemory,
  resetAllocator,
  copyIn,
 ...
 */
export function wasmSinh(a: ArrayStorage): ArrayStorage | null {
  ...
  if (dtype === 'int64' || dtype === 'uint64') {
    ...
    (dtype === 'int64' ? sinh_i64 : sinh_u64)(aPtr, outPtr, size);
    const outData = copyOut(
      outPtr,
      size,
  ...
  // (NumPy promotes int→float64 for sinh)
  const bpe = 8; // f64
  ensureMemory(size * bpe * 2);
  resetAllocator();
