 ...
 */
export function wasmArccos(a: ArrayStorage): ArrayStorage | null {
  ...
  if (dtype === 'float16') {
    ...
    arccos_f32(aPtr, outRegion.ptr, size);
  ...
  if (dtype === 'int64' || dtype === 'uint64') {
    ...
    (dtype === 'int64' ? arccos_i64 : arccos_u64)(aPtr, outRegion.ptr, size);
  ...
  // (NumPy promotes int→float64 for arccos)
  const bpe = 8; // f64
  const outBytes = size * bpe;
  ...
  arccos_f64(aPtr, outRegion.ptr, size);
