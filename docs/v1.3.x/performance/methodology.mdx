...
Both sides use high-resolution timers: `performance.now()` in JavaScript and `time.perf_counter()` in Python. The benchmark measures **computation time only**, from the JS side for numpy-ts and from the Python side for NumPy. This gives an apples-to-apples comparison of the numerical computation itself, without being skewed by JS↔Python interop overhead.
